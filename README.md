...
## Overview
**AdaL1TV** is an inversion analysis tool designed to estimate 3D subsurface magnetization (or magnetic susceptibility) models from observed magnetic anomaly data. Adopting **ADMM (Alternating Direction Method of Multipliers)** as the optimization method, it provides the following advanced regularization techniques that excel at recovering sharp geological boundaries.
...
* **Fast Large-Scale Computation**: 
  * Implements multi-threaded processing using **OpenMP** for tasks like constructing 3D spatial differential operators.
...
* **Geophysical Modeling**: Allows arbitrary settings for the inclination and declination of the external geomagnetic field and the magnetization vector. Supports forward modeling kernel computation using 3D prism grids that can incorporate surface topography (terrain) models.
...
* **External Libraries**: `mgcal` (for magnetic calculations) and `mmreal` (for matrix and vector operations) are included in the source tree.
...
This is the main observation data specified with the `-f` option. Each row must contain the following 4 values:
```text
<Easting (km)>    <Northing (km)>    <Altitude (km)>    <Magnetic Anomaly (nT)>
```

### 2. Terrain Data
Provide this file when incorporating surface topography. Each row must contain the following 3 values:
```text
...
Required Arguments:
  ...
  -l <log10(lambda)>    Common logarithm value of the regularization parameter (λ)

Optional Arguments:
  -a <alpha>            Mixing ratio of L1 and L2 norms
```
...
```bash
USAGE: ./l1tvinv -f <input_file> [optional arguments]
...
Optional Arguments:
  -l <log10(lambda)>    Common logarithm value of the regularization parameter (λ)
  - guide_model_file    Path to the guide model file for adaptive weighting
```

## Outputs
Upon successful completion, the program will generate the following files:
