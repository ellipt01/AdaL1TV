...
* **External Libraries** (Included in the source tree): 
  * `mgcal`: For magnetic forward calculations.
  * `mmreal`: A library for handling sparse and dense matrices/vectors in the standard **[Matrix Market format](https://math.nist.gov/MatrixMarket/formats.html)** (see the [NIST Matrix Market](https://math.nist.gov/MatrixMarket) for details).
...
Optional Arguments:
  -g <guide_model_file>:<sigma>:<c1>:<c2>
                        - guide_model_file: guide model in [Matrix Market format](https://math.nist.gov/MatrixMarket/formats.html)
...
* `beta_L1L2.vec` / `beta_L1TV.vec`: The model vector file before weight removal, output in the standard **[Matrix Market format](https://math.nist.gov/MatrixMarket/formats.html)** (via the `mmreal` library).
